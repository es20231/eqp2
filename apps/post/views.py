...
@login_required(login_url='/autenticacao/login/')
def publicar2(request):
    return HttpResponse("Placeholder para publicar2")
    """Função que publica um post do usuário sem usar o request.FILES"""
    # if request.method =="GET":
    #     return render(request, 'post/novo-post.html')
    
    # else:
    #     id_post = request.POST.get('id')
    #     imagem_post = request.POST["acao"]
    #     data_de_publicacao_post = request.POST.get('data_de_publicacao')
